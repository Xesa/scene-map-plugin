@tool
class_name NodePreviewer extends SceneMapHelper

## Creates the preview box and calls the [_refresh_preview] method.
static func create_preview(node : SceneMapNode) -> void:
...
static func refresh_preview(node : SceneMapNode) -> void:

	# Creates a subviewport that will hold the scene
	var viewport := SubViewport.new()
	viewport.size = node.VIEWPORT_SIZE
	...
	camera.enabled = true
	camera = fit_camera_to_scene(camera, scene_size, node.VIEWPORT_SIZE)
	...
	node.preview.texture = ImageTexture.create_from_image(image)
